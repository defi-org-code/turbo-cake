...
describe("BaseTest", function () {
  it("Base Test", async function () {
	...
	console.log(`totalCakesInit=${totalCakesInit}`);

	await cake.methods.transfer(accounts[0], totalCakesInit).send({from: cakeWhale});

	const botTotalCakes = await cake.methods.balanceOf(accounts[0]).call();

	console.log(`botTotalCakes=${botTotalCakes}`);

	  // console.log(cake.abi);
	// await cake.transferFrom(accounts[0], 10000, {from: cakeWhale});
